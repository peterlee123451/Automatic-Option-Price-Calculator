...
getSymbols("DTB3", src = "FRED", from = Exit, to = Today)
US_Rate <- log(1+DTB3[length(DTB3)])

getSymbols("IRLTLT01GBM156N", src = "FRED")
UK_Rate <- log(1+IRLTLT01GBM156N[length(IRLTLT01GBM156N)])

getSymbols("IR3TIB01AUM156N", src = "FRED")
AU_Rate <- log(1+IR3TIB01AUM156N[length(IR3TIB01AUM156N)])

#Output
DATA1 <- data.frame(US_Rate,UK_Rate,AU_Rate)
write.csv(DATA1, file = "Output1.csv", row.names = FALSE)

DATA2 <- data.frame(Por_SD_Output, t(Ind_SD_Output))
write.csv(DATA2, file = "Output2.csv", row.names = FALSE)
